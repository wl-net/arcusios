//  xCopyright 2016 Skyscanner Ltd
//
...
open class SkyFloatingLabelTextField: UITextField {
  ...
  open func lineViewRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
    var lineHeight: CGFloat = CGFloat(self.lineHeight)
    if editing {
      lineHeight = CGFloat(self.selectedLineHeight)
    }
  ...
  override open func layoutSubviews() {
    ...
    self.titleLabel.frame =
      self.titleLabelRectForBounds(self.bounds,
                                   editing: self.isTitleVisible() || _renderingInInterfaceBuilder)
    self.lineView.frame =
      self.lineViewRectForBounds(self.bounds,
                                 editing: self.editingOrSelected || _renderingInInterfaceBuilder)
  }
